...
Note!!!!! In the manuscript, the additive effect was described as being sampled from a normal distribution. In practice, however, AlphaSimR samples the effects and then scales them to achieve the target additive variance in the founder population. The statement in the manuscript was meant as a simplified explanation to make the process easier for readers to follow. Under the assumption that `p=0.5` for all loci and each locus is independent, the expected additive variance is `0.5×nloci×σa2`. Thus, if the target variance is `5.39` with `nloci=4400`, then σa2 should be `5.39/(0.5×4400)`. The sentence in the manuscript overlooked this 0.5 factor😅!
